...
class VideoFrameFirst : VideoFrame{
    ...
    func setModel() {
        virtualObjectScene = SCNScene(named: "video_frame.dae", inDirectory: "art.scnassets")
        
        guard let virtualObject = virtualObjectScene else { return }
        let pictureNode = virtualObject.rootNode.childNode(withName: "SketchUp", recursively: true)
        
        if let albumNode = pictureNode?.childNode(withName: "ID281", recursively: false) {
            ...
            let material_L = albumNode.geometry?.material(named: "_2")
